...
console.log('5.a:');
var colors=['green', 'yellow','pink','lightblue','purple'];
for (var i=0; i<=colors.length-1;i++){
    alert(colors[i]);
}
/*Al array anterior convertir la primera letra de cada palabra en 
mayúscula y mostrar una alerta por cada palabra modificada.*/
console.log('5.b:');
var colors=['green', 'yellow','pink','lightblue','purple'];
for (var i=0; i<=colors.length-1;i++){
    alert(colors[i].substring(0,1).toUpperCase()
    +colors[i].substring(1,colors[i].length).toLowerCase());
}
